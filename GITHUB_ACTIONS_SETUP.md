...
This guide explains how to set up automated robot data scraping every 3 hours using GitHub Actions.

## What It Does

The GitHub Actions workflow (`scrape-robots.yml`) automatically:
1. Runs every 3 hours
2. Scrapes live data from all 9 TCS MASTER sites
3. Merges live data with static backup data
4. Commits changes to the repository
5. Deploys updated data to Netlify
...
**Secret 3 (for manual refresh button):**
- Name: `GITHUB_TOKEN`
- Value: Create a Personal Access Token:
  1. Go to GitHub Settings → Developer settings → Personal access tokens → Tokens (classic)
  2. Click "Generate new token (classic)"
  3. Name it: "TCS Robot Registry"
  4. Select scope: `repo` (full control of private repositories)
  5. Click "Generate token"
  6. Copy the token

**Secret 4 (for manual refresh button):**
- Name: `GITHUB_REPO`
- Value: `your-username/your-repo-name` (e.g., `johndoe/tcs-robot-registry`)

**Note**: Secrets 3 & 4 are required for the manual "Refresh Live Data" button on the website to work.

### 3. Push the Workflow File

The workflow file is already created at `.github/workflows/scrape-robots.yml`.

Just commit and push:

```bash
git add .github/workflows/scrape-robots.yml
git commit -m "Add GitHub Actions workflow for robot scraping"
git push
```
